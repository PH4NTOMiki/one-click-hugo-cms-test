...
const actions = {
  ...
  deleteNominee: async ({ request, locals }) => {
    const f = await request.formData();
  ...
  setWinner: async ({ request, locals }) => {
    ...
    const { error } = await locals.supabase.from("stories").update({ is_winner: true, status: "approved" }).eq("id", id);
    if (error) return fail(500, { error: "Postavljanje pobjednika nije uspjelo." });
    return { success: true };
  },
  clearWinner: async ({ request, locals }) => {
    const f = await request.formData();
    const { error } = await locals.supabase.from("stories").update({ is_winner: false }).eq("id", String(f.get("id")));
