...
const load = async ({ locals }) => {
  const { data: stories } = await locals.supabase.from("stories").select("id, nurse_name, workplace, city, message, is_winner, created_at").or("status.eq.approved,is_winner.eq.true").order("is_winner", { ascending: false }).order("created_at", { ascending: false });
